...
export default function GemStoreScreen() {
	...
	const onPurchase = async (metadata: {
		...
		try {
			...
			Alert.alert(
				t('apps.purchase.gem_store.error_alert_title'),
	...
	const onError = async (error: unknown) => {
		...
		setShowPayment(false);
		showErrorModal(
			error instanceof Error ? error.message : t('apps.purchase.gem_store.error_alert_message'),
			'error',
		);
	};
	...
	return (
		<View testID="gem-store-screen" style={{ flex: 1 }}>
			<GemStore.Layout
				...
				renderMissionSection={() => (
					<GemStore.MissionSection missions={missions} onMissionPress={handleMissionPress} />
				)}
			/>
		</View>
	);
