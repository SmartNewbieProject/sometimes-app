...
import { Button, ImageSelector, PalePurpleGradient, Text } from '@/src/shared/ui';
...
const schema = z.object({
	...
	images: z.array(z.string().nullable()).min(3, i18n.t('apps.auth.reapply.validation.images_min')),
});
	...
	const onSubmit = () => {
		...
		handleSubmit(async (data) => {
			...
			try {
				...
				const errorMessage =
					error instanceof Error ? error.message : t('apps.auth.reapply.modal_reapply_fail_desc');
				showModal({
	...
	return (
		<DefaultLayout style={styles.container}>
			...
			<ScrollView style={styles.scrollView} contentContainerStyle={{ flexGrow: 1 }}>
				<View style={styles.contentWrapper}>
					...
					<View style={styles.sectionContainer}>
						...
						<Text size="sm" textColor="pale-purple" weight="light" style={styles.profileImageInfo}>
			...
			<View style={[styles.bottomContainer, { paddingBottom: insets.bottom + 32 }]}>
				...
				>
					{isLoading
						? t('apps.auth.reapply.button_reapply_loading')
						: t('apps.auth.reapply.button_next')}
				</Button>
			</View>
		</DefaultLayout>
	);
