...
export default function MilitarySelectionScreen() {
	...
	const preferences: Preferences =
		preferencesArray?.find((item) => item.typeCode === PreferenceKeys.MILITARY_STATUS) ??
		preferencesArray[0];
	...
	const saveAndRedirectForFemale = useCallback(async () => {
		const emptyFields = [];
	...
	useFocusEffect(
		useCallback(() => {
			if (my?.gender === 'FEMALE' || i18n.language === 'ja') {
				saveAndRedirectForFemale();
				return;
			}
			useMyInfoStep.getState().updateStep(MyInfoSteps.MILITARY);
		}, [my?.gender, i18n.language, saveAndRedirectForFemale]),
	);
