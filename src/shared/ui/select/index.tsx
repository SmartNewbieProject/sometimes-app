...
const select = cva('w-full flex flex-col justify-center bg-transparent border-b border-[#E7E9EC]', {
	variants: {
		size: {
			sm: 'h-10 text-sm',
			md: 'h-12 text-md',
...
export function Select({
	...
	return (
		<View className={cn(className)} style={width ? { width } : undefined}>
			<TouchableOpacity onPress={() => !isDisabled && setIsOpen(!isOpen)} activeOpacity={0.8}>
				<View className={select({ size, status, isDisabled })}>
					<Text size={size} textColor={value ? 'black' : 'pale-purple'} className="py-2">
					...
					>
						{options.map((option) => (
							<TouchableOpacity
								...
								className={cn('py-3 px-4', value === option.value && 'bg-lightPurple')}
								style={{
									width: '100%',
								}}
							>
