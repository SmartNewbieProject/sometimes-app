...
export function StepSlider({
	...
	const handleStepClick = (direction: 'prev' | 'next') => {
		...
		const newValue =
			direction === 'next' ? Math.min(max, value + step) : Math.max(min, value - step);
...
export function FormStepSlider({ name, control, rules, ...props }: FormStepSliderProps) {
	...
	return <StepSlider value={value} onChange={onChange} {...props} />;
}
