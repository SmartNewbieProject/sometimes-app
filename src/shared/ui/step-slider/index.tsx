...
export function StepSlider({
	...
	const handleDragStart = useCallback(
		(event: GestureResponderEvent) => {
			...
			if (newValue !== null) {
				...
				draggingValueRef.current = newValue;
	...
	const handleDragEnd = useCallback(() => {
		...
		if (draggingValueRef.current !== null) {
			handleValueChange(draggingValueRef.current);
		}
		isDraggingRef.current = false;
		setDraggingValue(null);
		draggingValueRef.current = null;
	}, [handleValueChange, throttledDragMove]);
	...
	return (
		<View
			style={[styles.container, style, disabled && { opacity: 0.3 }]}
						...
						>
							{bubbleLabel ? (
								<View style={styles.bubble}>
									<Text size="13" weight="semibold" style={styles.bubbleText}>
										{bubbleLabel}
									</Text>
...
const styles = StyleSheet.create({
	...
	bubbleArrow: {
		...
		borderTopColor: semanticColors.brand.primary,
	},
});
