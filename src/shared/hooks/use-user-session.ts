...
export const useUserSession = (userId?: string) => {
	...
	const startUserSession = useCallback(
		(
			sessionType: 'new' | 'returning' | 'background_return',
			source: 'direct' | 'push' | 'deeplink' = 'direct',
		) => {
			...
			// 앱 오픈 이벤트 발송
			mixpanelAdapter.track(MIXPANEL_EVENTS.APP_OPENED, {
			...
			// 세션 시작 이벤트 (새 세션만)
			if (isNewSession) {
				mixpanelAdapter.track(MIXPANEL_EVENTS.SESSION_STARTED, {
					session_id: sessionRef.current.sessionId,
	...
	const startFeatureUsage = useCallback(
		(featureName: string) => {
			...
			mixpanelAdapter.track(MIXPANEL_EVENTS.FEATURE_USED, {
				session_id: sessionRef.current.sessionId,
	...
	// 기능 사용 종료
	const endFeatureUsage = useCallback(
		(featureName: string, additionalData?: any) => {
	...
	const endUserSession = useCallback(
		(endReason: 'app_close' | 'background_timeout' | 'manual_logout') => {
			const currentTime = Date.now();
	...
	const handleBackground = useCallback(() => {
		...
		mixpanelAdapter.track(MIXPANEL_EVENTS.APP_BACKGROUNDED, {
			session_id: sessionRef.current.sessionId,
	...
	}, [startUserSession, endUserSession]);
	...
	const getSessionMetrics = useCallback((): SessionMetrics => {
		...
		return {
			...
			backgroundDuration: backgroundStartTime.current
				? currentTime - backgroundStartTime.current
				: 0,
			featuresUsed: sessionRef.current.featuresUsed,
...
function assessSessionQuality(session: UserSessionData): 'excellent' | 'good' | 'fair' | 'poor' {
	...
	if (sessionDuration > 5 * 60 * 1000)
		...
		timeScore = 3; // 2-5분
	else if (sessionDuration > 30 * 1000)
		timeScore = 2; // 30초-2분
