...
export const useUserSession = (userId?: string) => {
  ...
  const startUserSession = useCallback((sessionType: 'new' | 'returning' | 'background_return', source: 'direct' | 'push' | 'deeplink' = 'direct') => {
    ...
    // 앱 오픈 이벤트 발송
    mixpanelAdapter.track(AMPLITUDE_KPI_EVENTS.APP_OPENED, {
    ...
    // 세션 시작 이벤트 (새 세션만)
    if (isNewSession) {
      mixpanelAdapter.track(AMPLITUDE_KPI_EVENTS.SESSION_STARTED, {
        session_id: sessionRef.current.sessionId,
  ...
  const startFeatureUsage = useCallback((featureName: string) => {
    ...
    mixpanelAdapter.track(AMPLITUDE_KPI_EVENTS.FEATURE_USED, {
      session_id: sessionRef.current.sessionId,
  ...
  const endUserSession = useCallback((endReason: 'app_close' | 'background_timeout' | 'manual_logout') => {
    ...
    // 세션 종료 이벤트 발송
    mixpanelAdapter.track(AMPLITUDE_KPI_EVENTS.SESSION_ENDED, {
      session_id: sessionRef.current.sessionId,
  ...
  const handleBackground = useCallback(() => {
    ...
    mixpanelAdapter.track(AMPLITUDE_KPI_EVENTS.APP_BACKGROUNDED, {
      session_id: sessionRef.current.sessionId,
  ...
  useEffect(() => {
    if (userId && sessionRef.current.userId !== userId) {
      ...
      // 로그인 이벤트 발송
      mixpanelAdapter.track(AMPLITUDE_KPI_EVENTS.AUTH_LOGIN_COMPLETED, {
        session_id: sessionRef.current.sessionId,
        user_id: userId,
