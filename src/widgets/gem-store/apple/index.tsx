...
import { extractNumber, getPriceAndDiscount, getPriceAndDiscountFromServer } from '../utils/apple';
					...
					<Show when={Boolean(priceAndDiscount && priceAndDiscount.discountRate > 0)}>
						<View style={{ display: 'flex', flexDirection: 'row', columnGap: 6 }}>
							<Text style={styles.originalPrice}>
								{toKRW(priceAndDiscount?.price ?? 0)}
								{t('widgets.gem-store.apple.unit_currency')}
							</Text>
							<Text style={styles.discountRate}>
								{priceAndDiscount?.discountRate}
								{t('widgets.gem-store.apple.discount_suffix')}
							</Text>
						</View>
					</Show>
			...
			{hot && (
				...
				>
					<Text style={{ color: semanticColors.text.inverse, fontSize: 10, fontWeight: 'bold' }}>
						{t('widgets.gem-store.apple.popular_badge')}
					</Text>
				</View>
			)}
		</Pressable>
