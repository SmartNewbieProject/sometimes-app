...
function Slider({
	...
	const onButtonNavigation = (index: number) => {
		if (isSingle) return;
		// autoPlay 애니메이션 중이어도 인디케이터 탭은 항상 허용
	...
	const panResponder = PanResponder.create({
		...
		onPanResponderGrant: () => {
			...
			bannerAnim.stopAnimation();
			pendingRef.current = true;
			baseOffsetRef.current = -focusRef.current * containerWidth;
		},
		onPanResponderMove: (_e, gestureState) => {
			// 드래그 중 실시간 시각적 피드백
			bannerAnim.setValue(baseOffsetRef.current + gestureState.dx);
		},
	...
	return (
		...
		>
			<View style={[{ width: containerWidth || 0, overflow: 'hidden' }, contentContainerStyle]}>
				<Animated.View
						...
						>
							<View
								style={[
									styles.indicator,
									{
										backgroundColor: focusIndex === index + 1 ? '#7A4AE2' : colors.lightPurple,
									},
