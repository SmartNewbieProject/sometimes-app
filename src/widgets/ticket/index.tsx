...
const TicketProvider = ({
	children,
	...props
}: { children: React.ReactNode } & TicketContextProps) => {
...
const TicketComponent = ({ count, hot, discount, onOpenPayment }: TicketDetails) => {
	...
	return (
		<TouchableOpacity
			style={[styles.container, hot && styles.hot]}
			onPress={() => onOpenPayment({ totalPrice: discountedPrice, count })}
			activeOpacity={0.7}
		>
			<Image source={require('@assets/images/ticket.png')} style={styles.ticket} />
			<View style={styles.details}>
				<Text textColor="black" size="13" weight="semibold">
					{name}
				</Text>
				<Text textColor="black" size="13" weight="semibold">
					{count}회권
			...
			<View style={styles.price}>
				<View style={styles.sales}>
					{discount && (
						<>
							<Text textColor="gray" className="text-[12px] line-through">
								{totalOriginPrice}원
							</Text>
			...
			<Show when={!!hot}>
				<View style={styles.hotCard}>
					<Text textColor="white" size="13" weight="semibold">
						인기
					</Text>
				</View>
			</Show>
