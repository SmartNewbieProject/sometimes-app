...
export const ArticleWriteForm = ({ mode = 'create' as 'create' | 'update' }) => {
							...
							allowedCategories.map((c) => {
								...
								return (
									<TouchableOpacity
										...
										accessibilityRole="button"
										accessibilityLabel={t('features.community.ui.category_selector.select_label', {
											name: c.displayName,
										})}
									>
										<View
	...
	const pickImage = async () => {
		if (images.length >= 5) {
			Alert.alert(
				t('features.community.ui.article_write_screen.form.alert_title'),
				t('features.community.ui.article_write_screen.form.alert_image_limit'),
			);
			return;
		}
				...
				<View style={formStyles.headerRow}>
					{allowedCategories.length > 0 && (
						<TouchableOpacity
							...
							style={[formStyles.categoryButton, { opacity: mode === 'create' ? 1 : 0.6 }]}
							disabled={mode !== 'create'}
						>
							<Text style={formStyles.categoryButtonText}>{displayName} ▼</Text>
						</TouchableOpacity>
					)}
					...
					<View style={formStyles.flex1}>
						<Controller
							...
							render={({ field: { onChange, value } }) => (
								<TextInput
									placeholder={t(
										'features.community.ui.article_write_screen.form.title_placeholder',
									)}
				...
				<View style={formStyles.contentContainer}>
					<Controller
						...
						render={({ field: { onChange, value } }) => (
							<TextInput
								placeholder={t(
									'features.community.ui.article_write_screen.form.content_placeholder',
								)}
								multiline
								scrollEnabled
					...
					<View style={formStyles.imagePickerRow}>
						<TouchableOpacity onPress={pickImage} style={formStyles.imagePickerButton}>
							<Image
							...
							<Text style={formStyles.imagePickerText}>
								{t('features.community.ui.article_write_screen.form.add_image_button')} (
								{images.length}/5)
							</Text>
						</TouchableOpacity>
								...
								<View style={formStyles.imagesRow}>
									{images.map((uri: string, index: number) => (
										<View key={`image-${uri}-${index}`} style={formStyles.imageWrapper}>
											<Image source={{ uri }} style={formStyles.previewImage} resizeMode="cover" />
											<TouchableOpacity
