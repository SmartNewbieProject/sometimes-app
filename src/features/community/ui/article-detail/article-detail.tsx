	...
	const like = (item: Article) => {
		tryCatch(
			async () => {
				...
				queryClient.setQueriesData({ queryKey: QUERY_KEYS.articles.lists() }, (oldData: any) => {
					if (!oldData) return oldData;
	...
	return (
		<View className="flex-1 relative mb-12 px-5">
			...
			<View>
				<Text
					weight="medium"
					className="ml-[8px] text-md md:text-[18px] mb-[5px]"
					textColor="black"
				>
					{article.title}
				</Text>
			...
			>
				<Loading.Lottie title="댓글을 불러오고 있어요" loading={isCommentLoading}>
					<View className="flex flex-col gap-y-[8px] mt-2">{renderComments(comments)}</View>
				</Loading.Lottie>
			</ScrollView>
