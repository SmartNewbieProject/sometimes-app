...
export default function Notice({ pageSize = 5 }: Props) {
	...
	const { notices, isLoading, isError, error, index, total, refs, handlers, containerWidth } =
			...
			<View style={styles.container} onLayout={handlers.onLayout}>
				...
				>
					{notices.slice(0, pageSize).map((notice) => (
						<View key={notice.id} style={{ width: containerWidth || undefined }}>
							<TouchableOpacity activeOpacity={0.9} onPress={() => handlePressArticle(notice.id)}>
								<Article
									data={notice}
