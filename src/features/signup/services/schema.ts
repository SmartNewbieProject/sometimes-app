...
export const accountSchema = z.object({
	...
	password: z
		...
		.regex(passwordRegex, { message: '영문, 숫자, 특수문자 조합 8자리 이상을 입력해주세요.' }),
	passwordConfirm: z
...
export const phoneSchema = z.object({
	phoneNumber: z
		...
		.min(11, { message: '올바른 휴대폰 번호를 입력해주세요.' }),
	authorizationCode: z
		.string({ required_error: '인증번호를 입력해주세요.' })
		.min(6, { message: '올바른 인증번호를 입력해주세요.' }),
});
...
export const profileSchema = z.object({
	...
	mbti: z
		.string({ required_error: 'MBTI를 선택해주세요' })
		.min(4, { message: 'MBTI를 선택해주세요' }),
});
