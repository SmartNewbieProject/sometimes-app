...
export type ChatSocket = Socket<ChatServerToClientEvents, ChatClientToServerEvents> | null;

export type ChatEventActions = {
	sendMessage: (payload: { to: string; content: string; chatRoomId: string; senderId: string }) => {
	...
	uploadImage: (payload: {
		...
		senderId: string;
		file: any;
	}) => Promise<{ optimisticMessage: Chat; promise: Promise<any> }>;
	readMessages: (chatRoomId: string) => void;
};

export const createChatEventActions = (getSocket: () => ChatSocket): ChatEventActions => ({
