...
function ChatScreen() {
	...
	useEffect(() => {
		return () => {
			if (id && chatList.length > 0) {
				...
				chatEvents.trackChatEnded(id, chatDuration, totalMessageCount, 'screen_exit');
			}
		};
	...
	const animatedStyles = useAnimatedStyle(() => ({
		transform: [
			{
				translateY: Platform.OS === 'android' ? 0 : -keyboard.height.value + insets.bottom,
			},
		],
	}));
		...
		>
			...
			>
				...
				{Platform.OS === 'web' ? (
					...
					<ChatInput isPhotoClicked={isPhotoClicked} setPhotoClicked={setPhotoClicked} />
				)}
