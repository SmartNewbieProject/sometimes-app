...
import { LIKE_KEYS } from '@/src/shared/libs/locales/keys';
	...
	return imagesUrls && imagesUrls?.length > 0 ? (
		<View style={styles.gradientContainer}>
			...
			<Text style={styles.text}>
				{type === 'iLiked'
					? t(LIKE_KEYS.ui.likeCollapse.iLikedTitle, { name })
					: t(LIKE_KEYS.ui.likeCollapse.likedMeTitle, { name })}
			</Text>
			<Pressable
				style={styles.container}
				onPress={() => {
					type === 'iLiked' ? router.push('/post-box/i-liked') : router.push('/post-box/liked-me');
				}}
			>
				<LinearGradient
					start={[0, 1]}
				...
				<View style={styles.contentContainer}>
					<ImageCollapse
						...
						imageUrls={imagesUrls.length > 5 ? imagesUrls.slice(0, 5) : imagesUrls}
					/>
				</View>
				{imagesUrls.length > 5 && (
					<View style={styles.more}>
						<Text style={[styles.moreText]}>{`+${Math.abs(5 - imagesUrls.length)}`}</Text>
					</View>
				)}
			</Pressable>
		</View>
