...
const sendMessageStreamWeb = async (
	...
	const response = await fetch(`${baseURL}${AI_CHAT_BASE_URL}/sessions/${sessionId}/messages`, {
		method: 'POST',
		headers: {
	...
	if (!response.ok) {
		...
		throw new Error(error.message || i18n.t('apis.메시지_전송에_실패했습니다'));
	}
	...
	if (!reader) {
		throw new Error(i18n.t('apis.스트림을_읽을_수_없습니다'));
	}
	...
	return new Promise((resolve, reject) => {
		...
		es.addEventListener('error', (error) => {
			...
			if (error && typeof error === 'object' && 'message' in error) {
				...
				reject(new Error(i18n.t('apis.메시지_전송에_실패했습니다')));
			}
		});
...
export const analyzeSession = async (data?: AnalyzeRequest): Promise<AnalyzeResponse> => {
