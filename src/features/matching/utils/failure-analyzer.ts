...
export const determineFailureReason = (error: any): FailureReason => {
	...
	if (status === 404) {
		if (
			message.includes('매칭_연결을_찾을_수_없습니다') ||
			message.includes('매칭을_찾을_수_없습니다')
		) {
			return {
		...
		if (
			message.includes('대상_사용자를_찾을_수_없습니다') ||
			message.includes('사용자를_찾을_수_없습니다')
		) {
			return {
				type: 'USER_NOT_FOUND',
				category: 'SYSTEM',
...
const getFailurePreventionActions = (riskType: string, _context: FailureContext) => {
	const actions: Record<
		string,
		{ action: string; messageKey: string; buttonKey: string; priority: string }
	> = {
		TICKET_INSUFFICIENT: {
