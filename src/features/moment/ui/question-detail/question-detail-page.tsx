...
import { ArrowLeft, Check, Heart, List, Loader2, PenTool, Sparkles } from 'lucide-react-native';
...
export const QuestionDetailPage = () => {
				...
				showModal({
					...
					children: (
						<Text size="14" weight="normal" textColor="dark">
							{t('features.moment.question_detail.modal.no_multiple_choice')}
						</Text>
					),
					primaryButton: {
	...
	const handleGetInspiration = async () => {
		...
		try {
			...
			trackQuestionAIInspirationApply({
				question_id: question.id,
				suggestion_length: inspiration.length,
			});
		} catch (error) {
	...
	const handleSaveAnswer = async () => {
		...
		const isValid =
			questionType === 'text' ? textAnswer.trim().length > 0 : selectedOptionData !== null;
			...
			>
					...
					options={{
						...
						headerLeft: () => (
							<TouchableOpacity
								...
								accessibilityLabel={t('features.moment.question_detail.back')}
							>
								<ArrowLeft size={24} color={semanticColors.text.primary} accessible={false} />
							</TouchableOpacity>
						),
					}}
				/>
				...
				{questionLoading ? (
					...
					<View style={styles.errorContainer}>
						...
						{/* Debug 정보 */}
						<View
							style={{ marginTop: 16, padding: 12, backgroundColor: '#f5f5f5', borderRadius: 8 }}
						>
					...
					<View style={styles.contentContainer}>
						...
						{step === 'envelope' && (
							<Envelope onPress={handleOpenLetter} questionDate={questionDate} />
						)}
