...
// 레이더 차트 컴포넌트
const RadarChart = ({ radarData }: { radarData: LatestReport['radarData'] }) => {
  const size = width - 80;
  const center = size / 2;
...
export const OnboardingResult = () => {
  ...
  const report: LatestReport | null = useMemo(() => {
  ...
  const toggleInsight = (index: number) => {
    setExpandedInsights((prev) => ({
      ...prev,
      [`insight${index}`]: !prev[`insight${index}`],
    }));
  };
  ...
  const firstTitle = report.userTitles?.[0];
      ...
      >
        ...
        {report.radarData && report.radarData.length > 0 && (
          <View style={styles.chartSection}>
            <RadarChart radarData={report.radarData} />
          </View>
        )}
          ...
          <View style={styles.insightsSection}>
            {report.insights.map((insight, index) => (
              <AnalysisCard
                ...
                title={`인사이트 ${index + 1}`}
                mode="toggle"
                isExpanded={expandedInsights[`insight${index}`]}
                onToggle={() => toggleInsight(index)}
              >
                <Text size="14" weight="normal" textColor="black" style={styles.insightText}>
                  {insight}
                </Text>
              </AnalysisCard>
            ))}
