...
export const useCardNewsInfiniteList = (limit = 10) => {
	const query = useInfiniteQuery({
		...
		queryFn: ({ pageParam }) => getCardNewsList({ cursor: pageParam, limit }),
	...
	// 모든 페이지에서 데이터 평탄화
	const items: CardNewsListItem[] = query.data?.pages.flatMap((page) => page.data) ?? [];
...
export const prefetchCardNews = (queryClient: QueryClient) => {
