...
function MatchingHistoryCard({ item }: MatchingHistoryCardProps) {
	...
	const size =
		Dimensions.get('window').width > 468 ? 218 : (Dimensions.get('window').width - 32) / 2;
	...
	const onClickToPartner = () => {
		return router.navigate({
			...
			params: { id: item.matchId, redirectTo: encodeURIComponent('/matching-history') },
		});
	};
				...
				customTitle: (
					<View style={ModalStyles.title}>
						<Text textColor="black" size="20" weight="bold">
							{t('features.matching-history.ui.matching_history_card.modal_title', {
								cost: featureCosts?.PROFILE_OPEN,
							})}
						</Text>
					</View>
				),
						...
						<View style={styles.fullWidthRow}>
							<Pressable
								style={[
									...
									{
										backgroundColor: item.blinded ? '#452D79' : colors.primaryPurple,
									},
								]}
							...
							>
								<Text textColor="white" style={styles.buttonText}>
									{item.blinded
										? t('features.matching-history.ui.matching_history_card.profile_unlock_button')
										: t('features.matching-history.ui.matching_history_card.more_button')}
								</Text>
