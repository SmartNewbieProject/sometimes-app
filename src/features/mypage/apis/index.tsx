import type { Article as MyArticle, MyComment, MyLike } from '@/src/features/community/types';
...
export type MissedNotificationCount = {
	total: number;
	matching: number;
	likes: number;
	messages: number;
...
function extractPayload(raw: any) {
	...
	const lvl2 = lvl1 && typeof lvl1 === 'object' && 'data' in lvl1 ? lvl1.data : lvl1;
	return lvl2 ?? lvl1;
}
...
const getMissedNotificationCount = async (): Promise<MissedNotificationCount> => {
	return await axiosClient.get('/notifications/missed-count');
};
...
const uploadProfileImage = async (image: string, order: number): Promise<void> => {
...
const uploadProfileImages = async (images: string[]): Promise<void> => {
	...
	const filePromises = images
		...
		.map((imageUri) => createProfileFileObject(imageUri, `profile-${nanoid(6)}.png`));
	...
	if (data && Array.isArray(data.items) && data.meta) {
		...
		return {
			...
			meta: {
				...
				hasPreviousPage: Boolean(m.hasPreviousPage ?? Number(m.currentPage ?? fallback.page) > 1),
			},
		};
	}
...
const mypageApis: MyPageApis = {
	...
	async getMyComments({ page, size }) {
		const resp = await axiosClient.get<unknown>('/articles/my-commented-articles', {
			params: { page, size },
		});
		const payload = extractPayload(resp);
...
type Service = {
	...
	getMissedNotificationCount: () => Promise<MissedNotificationCount>;
